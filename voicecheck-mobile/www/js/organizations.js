...
async function viewOrganization(orgId) {
    ...
    try {
        const [orgRes, statsRes, membersRes, deptsRes] = await Promise.all([
            fetch(`${API_BASE}/organizations/${orgId}`, { headers }),
            fetch(`${API_BASE}/organizations/${orgId}/stats`, { headers }),
            fetch(`${API_BASE}/organizations/${orgId}/members`, { headers }),
            fetch(`${API_BASE}/organizations/${orgId}/departments`, { headers }).catch(() => ({ ok: false }))
        ]);
...
async function submitAddMemberForm(e) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/organizations/${viewingOrgId}/add-member`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('access_token')
        ...
        if (deptId && newMember.id) {
            await fetch(`${API_BASE}/organizations/${viewingOrgId}/departments/${deptId}/members`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + localStorage.getItem('access_token')
...
async function changeMemberRole(orgId, userId, newRole) {
    try {
        const response = await fetch(`${API_BASE}/organizations/${orgId}/members/${userId}/role`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('access_token')
...
async function removeMember(orgId, userId, memberName) {
    ...
    try {
        const response = await fetch(`${API_BASE}/organizations/${orgId}/members/${userId}`, {
            method: 'DELETE',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('access_token') }
        });
...
async function submitCreateDeptForm(e) {
    ...
    try {
        const response = await fetch(`${API_BASE}/organizations/${viewingOrgId}/departments`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('access_token')
...
async function submitEditDeptForm(e) {
    ...
    try {
        ...
        const res = await fetch(`${API_BASE}/organizations/${viewingOrgId}/departments/${deptId}`, {
            method: 'PUT',
            headers,
            body: JSON.stringify({ name, head_user_id: headUserId })
        });
        ...
        for (const id of currentIds) {
            if (!targetIds.has(id)) {
                await fetch(`${API_BASE}/organizations/${viewingOrgId}/departments/${deptId}/members/${id}`, {
...
async function deleteDepartment(deptId) {
    ...
    try {
        await fetch(`${API_BASE}/organizations/${viewingOrgId}/departments/${deptId}`, {
            method: 'DELETE',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('access_token') }
        });
        await viewOrganization(viewingOrgId);
